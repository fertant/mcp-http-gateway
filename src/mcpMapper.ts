...
function mapOpenApiTypeToJsonSchemaType(openApiSchema?: OpenAPIV3.SchemaObject): { type: JSONSchema7TypeName | undefined, format?: string, nullable?: boolean } {
    if (!openApiSchema || !openApiSchema.type) {
        if ((openApiSchema as Object).hasOwnProperty('anyOf')) {
            for (const variant of openApiSchema?.anyOf || []) {
                const prop: JSONSchema7 = (variant as JSONSchema7);
                if (prop.hasOwnProperty('type') && prop.type !== null) {
                    return { type:  (prop.type as JSONSchema7TypeName) };
                }
            }
        }
...
export function mapOpenApiToMcpTools(openapi: ProcessedOpenAPI): MappedTool[] {
    ...
    for (const path in openapi.paths) {
        ...
        for (const method in pathItem) {
            ...
            const operationId = operation?.operationId || path.slice(1).replace(new RegExp(`[/]`, 'gi'), '_').replace(new RegExp(`[{}]`, 'gi'), '');
