...
const argv = yargs(hideBin(process.argv))
    ...
    .option('description', {
        type: 'string',
        description: 'Short description of the MCP functionality'
    })
    .help()
...
const envValues = {
    ...
    pathDepth: process.env.PATH_DEPTH,
    ...
    presetParams: process.env.MCP_PRESET_PARAMS,
...
const interfaceType = getValueWithPriority(argv.type, envValues.type, jsonConfig.type, '');
const transport = getValueWithPriority(argv.transport, envValues.transport, jsonConfig.transport, '');
const pathDepth = getValueWithPriority(argv.pathDepth, envValues.pathDepth, jsonConfig.pathDepth, '');
...
const presetParams = getValueWithPriority(argv.presetParams, envValues.presetParams, jsonConfig.presetParams, '');
...
const description = getValueWithPriority(argv.description, envValues.description, jsonConfig.description, '');

if (interfaceType == 'openapi' && !specPath) {
...
export const config = {
    ...
    filter: {
        ...
        presetParams: presetParams ? presetParams : {},
    },
